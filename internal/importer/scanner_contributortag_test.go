...
func TestPrimaryContributor(t *testing.T) {
	cases := []struct {
		name string
		in   string
		want string
	}{
		{"daize contributor list", daizeArtistTag, "Álvaro Enrigue"},
		{"role suffix on the first segment", "Gabriel Porras - narrator, Álvaro Enrigue", "Álvaro Enrigue"},
		{"narrator credit segment", "Read by Nigel Planer, Terry Pratchett", "Terry Pratchett"},
		{"plain co-authors keep the first", "Neil Gaiman, Terry Pratchett", "Neil Gaiman"},
		...
		{"sort-form name is not a contributor list", "Enrigue, Álvaro", ""},
		{"sort-form with initial", "Martin, George R. R.", ""},
		{"no comma at all", "Álvaro Enrigue", ""},
		{"empty", "", ""},
		// Every segment is a credit: nothing usable, so don't guess.
		{"only credits", "Read by A. B., Someone Else - translator", ""},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			if got := primaryContributor(c.in); got != c.want {
				t.Errorf("primaryContributor(%q) = %q, want %q", c.in, got, c.want)
			}
		})
	}
}
