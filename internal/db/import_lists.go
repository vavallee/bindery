...
func (r *ImportListRepo) List(ctx context.Context) ([]models.ImportList, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT id, name, type, url, api_key, COALESCE(account, ''), root_folder_id, quality_profile_id,
		       monitor_new, auto_add, enabled, media_type, owner_user_id, last_sync_at, created_at, updated_at
...
func (r *ImportListRepo) Create(ctx context.Context, il *models.ImportList) error {
	...
	result, err := r.db.ExecContext(ctx, `
		INSERT INTO import_lists (name, type, url, api_key, account, root_folder_id, quality_profile_id,
		                          monitor_new, auto_add, enabled, media_type, owner_user_id, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		il.Name, il.Type, il.URL, il.APIKey, il.Account, il.RootFolderID, il.QualityProfileID,
		il.MonitorNew, il.AutoAdd, il.Enabled, il.MediaType, il.OwnerUserID, now, now)
	if err != nil {
...
func (r *ImportListRepo) Update(ctx context.Context, il *models.ImportList) error {
	...
	_, err := r.db.ExecContext(ctx, `
		UPDATE import_lists SET name=?, type=?, url=?, api_key=?, account=?, root_folder_id=?,
		                        quality_profile_id=?, monitor_new=?, auto_add=?, enabled=?, media_type=?, owner_user_id=?, updated_at=?
		WHERE id=?`,
		il.Name, il.Type, il.URL, il.APIKey, il.Account, il.RootFolderID, il.QualityProfileID,
		il.MonitorNew, il.AutoAdd, il.Enabled, il.MediaType, il.OwnerUserID, now, il.ID)
	if err != nil {
...
func scanImportList(rows *sql.Rows) (models.ImportList, error) {
	...
	err := rows.Scan(
		...
		&monitorNew, &autoAdd, &enabled, &il.MediaType, &il.OwnerUserID,
