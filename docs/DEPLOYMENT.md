...
| `BINDERY_JOBS_DRAIN_GRACE` | `25` | Seconds to drain detached background jobs (ABS import, Grimmory sync, manual library scan, startup syncs) after the HTTP server has stopped and before the database is closed. These jobs are cancelled at drain start and given this window to wind down cleanly; any still running when it expires are logged and the process proceeds to shut down. Accepts a Go duration (e.g. `45s`, `2m`). Size `BINDERY_SHUTDOWN_GRACE` + this below your Kubernetes `terminationGracePeriodSeconds` so drains finish before SIGKILL. |
...
**Graceful shutdown** — `BINDERY_SHUTDOWN_GRACE` (default `30s`) controls how long the server drains in-flight requests after SIGTERM. `kubectl rollout restart` no longer drops in-flight requests. After request draining, detached background jobs (ABS import, Grimmory sync, manual library scan, startup syncs) are drained for up to `BINDERY_JOBS_DRAIN_GRACE` (default `25s`) before the database is closed, so a deploy mid-import no longer produces "database is closed" errors or (for Grimmory) re-uploads on the next run.
